...
SELECT
	...
	s.order_date, 
    m.join_date,
    DATE_ADD(m.join_date, INTERVAL 7 DAY) AS first_week_after_joining,
    me.product_name,
    me.price
FROM sales AS s
INNER JOIN members AS m
...
SELECT 
	customer_id,
	SUM(
		CASE WHEN order_date BETWEEN join_date AND first_week_after_joining THEN 2*10*price
			WHEN order_date NOT BETWEEN join_date AND first_week_after_joining AND product_name = 'sushi' THEN 2*10*price 
            WHEN order_date NOT BETWEEN join_date AND first_week_after_joining AND product_name <> 'sushi' THEN 10*price
